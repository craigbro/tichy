...
sub myRoutines {
    ...
    # Additions by Rob
    #
    if ($addressed && $message =~ /health/) {
      &say("$health");
    }
    if ($addressed && message =~ /ora/) {
      &say("Statement Processed...");
    }
    #end Additions by Rob
    ...
    if ($addressed && $message =~ /vindiesel\?/)
    {
	
	open( THING, "~/bot/src/vindiesel.sh |");	
	chomp( $contents = <THING> );
    	&say("$contents");
    }

    if ($addressed && $message =~ /webcheckout\?/)
    {
	
	open( THING, "~/bot/src/webcheckout.sh |");	
	chomp( $contents = <THING> );
    	&say("$contents");
    }
